...
tempWeeksToGet=10; %Number of previous weeks of weekly data that we want to retrieve
...
tempRemoteDataFilenames={...
    'MODWCW_%04d%03d_DAILY_AQUA_CHLORA_FG_1KM.%s',... %MODIS Daily file format
    'MODWCW_%04d%03d_7DAY_AQUA_CHLORA_FG_1KM.%s',...  %MODIS 7Day file format
    'VRSUCW_%04d%03d_DAILY_SNPP_CHLORA_FG_750M.%s',... %VIIRS Daily file format
    'VRSUCW_%04d%03d_7DAY_SNPP_CHLORA_FG_750M.%s'}; %VIIRS 7Day file format
...
for i=1:numel(tempRemoteDataFileType)
    ...
    elseif strcmp(tempRemoteDataFileType{i},'7day')
        tempFilesToGet=compose(tempRemoteDataFilenames{i},repmat(repmat(tempTodayYear,tempWeeksToGet*7+1,1),numel(tempExtensionsToGet),1),  repmat((tempTodayDayOfYear:-1:tempTodayDayOfYear-tempWeeksToGet*7)',numel(tempExtensionsToGet),1),  string(reshape(repmat(tempExtensionsToGet,tempWeeksToGet*7+1,1),[],1)));
    end
